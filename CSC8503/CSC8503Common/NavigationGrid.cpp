...
NavigationGrid::NavigationGrid(const std::string& filename) : NavigationGrid() {
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			n.position = NCLVector3((float)(x * nodeSize), 0, (float)(y * nodeSize));
		}
	}
...
bool NavigationGrid::FindPath(const NCLVector3& from, const NCLVector3& to, NavigationPath& outPath) {
	...
	while (!openList.empty()) {
		...
		if (currentBestNode == endNode) {			//we've found the path!
			...
			while (node != nullptr) {
				...
				outPath.PushWaypoint(NCLVector3(node->position.x - offset, node->position.y, node->position.z - offset));
				node = node->parent;
			}
			return true;
		}
...
void NavigationGrid::DrawMap(GameTechRenderer* r) {
	for (int i = 0; i < (gridHeight * gridWidth) - gridWidth; ++i) {
		//r->DrawLine(allNodes[i].position, allNodes[i + 1].position, NCLVector4(0, 1, 0, 1));
		if (((i + 1) % gridWidth) != 0) {
			r->DrawLine(NCLVector3(allNodes[i].position.x - offset,
				allNodes[i].position.y + 30,
				allNodes[i].position.z - offset),
				NCLVector3(allNodes[i + 1].position.x - offset,
					allNodes[i + 1].position.y + 30,
					allNodes[i + 1].position.z - offset), NCLVector4(0, 1, 0, 1));

			r->DrawLine(NCLVector3(allNodes[i].position.x - offset,
				allNodes[i].position.y + 30,
				allNodes[i].position.z - offset),
				NCLVector3(allNodes[i + gridWidth].position.x - offset,
					allNodes[i + gridWidth].position.y + 30,
					allNodes[i + gridWidth].position.z - offset), NCLVector4(0, 1, 0, 1));
