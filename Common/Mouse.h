...
namespace NCL {
	...
	public:
		...
		inline NCLVector2	GetRelativePosition() const { return relativePosition; }
		//Get the window position of the mouse pointer
		inline NCLVector2	GetAbsolutePosition() const { return absolutePosition; }

		inline NCLVector2 GetWindowPosition() const {
	...
	protected:
		...
		void	SetAbsolutePosition(const NCLVector2& pos);

		//Set the absolute screen bounds (<0 is always assumed dissallowed). Used
		//by the window resize routine...
		void	SetAbsolutePositionBounds(const NCLVector2& bounds);
