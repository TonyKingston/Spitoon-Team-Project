...
NCLMatrix3::NCLMatrix3(float elements[9]) {
	array[0] = elements[0];
	array[1] = elements[1];
	array[2] = elements[2];
...
NCLMatrix3::NCLMatrix3(const NCLMatrix4 &m4) {
	array[0] = m4.array[0];
	array[1] = m4.array[1];
	array[2] = m4.array[2];
...
NCLMatrix3::NCLMatrix3(const NCLMatrix2 &m2) {
	array[0] = m2.array[0];
	array[1] = m2.array[1];
	array[2] = 0;
...
NCLMatrix3::NCLMatrix3(const Quaternion &quat) {
...
NCLVector3 NCLMatrix3::ToEuler() const {
	...
	if (testVal < 1.0f) {
		...
		return NCLVector3(psi1, theta1, phi1);
	}
	else {
		float phi	= 0.0f;	//x
		...
		return NCLVector3(Maths::RadiansToDegrees(psi), Maths::RadiansToDegrees(theta), Maths::RadiansToDegrees(phi));
	}
...
NCLMatrix3 NCLMatrix3::FromEuler(const NCLVector3 &euler) {
	NCLMatrix3 m;
...
NCLVector3 NCLMatrix3::GetRow(unsigned int row) const {
	assert(row < 3);
	int start = row;
	return NCLVector3(
		array[start],
...
void NCLMatrix3::SetRow(unsigned int row, const NCLVector3 &val) {
	assert(row < 3);
...
NCLVector3 NCLMatrix3::GetColumn(unsigned int column) const {
	assert(column < 3);
	int start = 3 * column;
	return NCLVector3(
		array[start],
...
void NCLMatrix3::SetColumn(unsigned int column, const NCLVector3 &val) {
	assert(column < 3);
	int start = 3 * column;
	array[start] = val.x;
	array[start + 1] = val.y;
...
NCLVector3 NCLMatrix3::GetDiagonal() const {
	return NCLVector3(array[0], array[4], array[8]);
}

void	NCLMatrix3::SetDiagonal(const NCLVector3 &in) {
	array[0] = in.x;
	array[4] = in.y;
	array[8] = in.z;
}

NCLVector3 NCLMatrix3::operator*(const NCLVector3 &v) const {
	NCLVector3 vec;
