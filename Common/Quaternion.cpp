...
Quaternion::Quaternion(const NCLVector3& vector, float w) {
	this->x = vector.x;
	this->y = vector.y;
	this->z = vector.z;
	this->w = w;
...
Quaternion::Quaternion(const NCLMatrix4& m) {
...
Quaternion::Quaternion(const NCLMatrix3& m) {
...
NCLVector3 Quaternion::ToEuler() const {
	NCLVector3 euler;
...
Quaternion NCL::Maths::Quaternion::FromToRotation(NCLVector3 aFrom, NCLVector3 aTo) {
	NCLVector3 axis = NCLVector3::Cross(aFrom, aTo);
	float angle = NCLVector3::Angle(aFrom, aTo);
	return Quaternion::AxisAngleToQuaterion(axis.Normalised(), angle);
}


NCLVector3		Quaternion::operator *(const NCLVector3& a)	const {
	Quaternion newVec = *this * Quaternion(a.x, a.y, a.z, 0.0f) * Conjugate();
	return NCLVector3(newVec.x, newVec.y, newVec.z);
}