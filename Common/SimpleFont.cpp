...
int SimpleFont::BuildVerticesForString(std::string &text, NCLVector2&startPos, NCLVector4&colour, float size, std::vector<NCLVector3>&positions, std::vector<NCLVector2>&texCoords, std::vector<NCLVector4>&colours) {
	int vertsWritten = 0;
	...
	for (size_t i = 0; i < text.length(); ++i) {
		...
		positions.emplace_back(NCLVector3(startPos.x + xStart, yStart + yOff, 0));
		positions.emplace_back(NCLVector3(startPos.x + xStart, yStart + yOff - yHeight, 0));
		positions.emplace_back(NCLVector3(startPos.x + xStart + charWidth, yStart + yOff - yHeight, 0));

		positions.emplace_back(NCLVector3(startPos.x + xStart + charWidth, yStart + yOff - yHeight, 0));
		positions.emplace_back(NCLVector3(startPos.x + xStart + charWidth, yStart + yOff, 0));
		positions.emplace_back(NCLVector3(startPos.x + xStart, yStart + yOff, 0));
		...
		texCoords.emplace_back(NCLVector2(charData.x0 * texWidthRecip, charData.y1 * texHeightRecip));
		texCoords.emplace_back(NCLVector2(charData.x0 * texWidthRecip, charData.y0 * texHeightRecip));
		texCoords.emplace_back(NCLVector2(charData.x1 * texWidthRecip, charData.y0 * texHeightRecip));

		texCoords.emplace_back(NCLVector2(charData.x1 * texWidthRecip, charData.y0 * texHeightRecip));
		texCoords.emplace_back(NCLVector2(charData.x1 * texWidthRecip, charData.y1 * texHeightRecip));
		texCoords.emplace_back(NCLVector2(charData.x0 * texWidthRecip, charData.y1 * texHeightRecip));
