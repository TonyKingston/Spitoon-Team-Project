...
Win32Window::Win32Window(const std::string& title, int sizeX, int sizeY, bool fullScreen, int offsetX, int offsetY)	{
	...
	size = NCLVector2((float)sizeX, (float)sizeY);
	fullscreenSize = NCLVector2(::GetDeviceCaps(::GetWindowDC(NULL), HORZRES), ::GetDeviceCaps(::GetWindowDC(NULL), VERTRES));
	defaultPosition = NCLVector2((float)offsetX, (float)offsetY);
	defaultSize = size;

	fullScreen ? position = NCLVector2(0, 0) : position = defaultPosition;
...
bool	Win32Window::InternalUpdate() {
	...
	winMouse->SetAbsolutePosition(NCLVector2((float)pt.x, (float)pt.y));
...
void	Win32Window::SetFullScreen(bool fullScreen) {
	NCLVector2 newSize, newPos;
	if (fullScreen)
	{
		newSize = fullscreenSize;
		newPos = NCLVector2(0, 0);
		...
		case (WM_MOUSEMOVE): {
			...
			if (mouse) {
				...
				realMouse->UpdateWindowPosition(
					NCLVector2((float)GET_X_LPARAM(lParam), (float)GET_Y_LPARAM(lParam))
				);
			}
