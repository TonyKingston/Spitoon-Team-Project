...
OGLRenderer::OGLRenderer(Window& w) : RendererBase(w)	{
	...
	debugLinesMesh->SetVertexPositions(std::vector<NCLVector3>(5000, NCLVector3()));
	debugLinesMesh->SetVertexColours(std::vector<NCLVector4>(5000, NCLVector3()));

	debugTextMesh->SetVertexPositions(std::vector<NCLVector3>(5000, NCLVector3()));
	debugTextMesh->SetVertexColours(std::vector<NCLVector4>(5000, NCLVector3()));
	debugTextMesh->SetVertexTextureCoords(std::vector<NCLVector2>(5000, NCLVector3()));
...
void OGLRenderer::DrawBoundMesh(int subLayer, int numInstances) {
	...
	if (numInstances > 1) {
		if (boundMesh->GetIndexCount() > 0) {
			...
			glDrawArraysInstanced(mode, offset, count, numInstances);
		}
	}
	else {
		if (boundMesh->GetIndexCount() > 0) {
...
void OGLRenderer::DrawString(const std::string& text, const NCLVector2&pos, const NCLVector4& colour, float size) {
	DebugString s;
	s.colour	= colour;
	s.pos		= pos;
	s.size		= size;
...
void OGLRenderer::DrawLine(const NCLVector3& start, const NCLVector3& end, const NCLVector4& colour) {
	DebugLine l;
	l.start		= start;
	l.end		= end;
	l.colour	= colour;
...
NCLMatrix4 OGLRenderer::SetupDebugLineMatrix() const {
	return NCLMatrix4();
}
NCLMatrix4 OGLRenderer::SetupDebugStringMatrix()const {
	return NCLMatrix4();
}

void OGLRenderer::DrawDebugData(bool isSplit) {
	...
	BindTextureToShader(font->GetTexture(), "fontTex", 0);
...
void OGLRenderer::DrawDebugStrings(bool isSplit) {
	vector<NCLVector3> vertPos;
	vector<NCLVector2> vertTex;
	vector<NCLVector4> vertColours;
...
void OGLRenderer::DrawDebugLines(bool isSplit) {
	vector<NCLVector3> vertPos;
	vector<NCLVector4> vertCol;
